...
const INNER_SYSTEM_PROMPT = [
  ...
  '2. 如任务涉及可复用方法，调用 skill.list() 查看是否有现成技能可用',
  '',
  '## 记忆与技能使用规则（重要）：',
  ...
  '3. 长内容分段写入（必须遵守，API 通道对大参数不可靠）：首次 write 创建文件，每段 ≤1500 字符；后续续写一律用 write 的 append=true 逐段追加。绝不能用普通 write 续写——那会整体覆盖之前的段落。需要重新生成完整文件时才用普通 write（覆盖大文件需 confirm=true）。确认文件末尾用 read 的 tail 参数',
  '4. 收到「参数在 API 传输中丢失/截断」的提示时：第 1 次可原样重试；再次出现必须立即改为小分段（≤1500 字符/段 + append=true），禁止第三次发送大参数',
  '5. Python 模块导入：同目录文件可直接导入，跨目录需用 sys.path.insert 或相对导入',
  '',
  '## 任务完成报告：',
  '1. 输出检索到的记忆列表',
  '2. 说明每条记忆如何影响了你的决策',
