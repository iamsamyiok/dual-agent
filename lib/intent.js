...
// ---------- 意图抽取 ----------
const INTENT_SCHEMA_HINT = `{"task":"一句话概述","goals":["目标，≤3条"],"deliverables":[{"path":"产出文件路径，非文件交付物为 null","criterion":"该交付物的验收标准"}],"constraints":["约束条件"],"acceptance":["可客观核验的验收条款，≤5条"]}`;
...
function buildIntentPrompt(taskText) {
  return [
    ...
    '4) goals ≤3 条、acceptance ≤5 条，直接输出 JSON，无任何解释文字。',
...
function normalizeIntent(raw) {
  ...
  const deliverables = (Array.isArray(raw.deliverables) ? raw.deliverables : [])
    ...
    .slice(0, 6)
    .map(d => ({
      path: (typeof d.path === 'string' && d.path.trim() && d.path.trim() !== 'null') ? d.path.trim().slice(0, 200) : null,
...
async function extractIntent(cfg, taskText, opts = {}) {
  try {
    ...
    return normalizeIntent(parseLooseJson(text));
  } catch { return null; }
}
...
module.exports = { parseLooseJson, callLLMText, extractIntent, normalizeIntent, formatIntentNote, buildVerifyArgs, buildJudgePrompt, parseVerdict };
