...
function audit(op, detail) {
  ...
  try { fs.mkdirSync(DATA_DIR, { recursive: true }); fs.writeFileSync(AUDIT_PATH, JSON.stringify(list, null, 2)); } catch { /* ignore */ }
}
...
function makeSnapshot(op, names) {
  ...
  for (const name of names) {
    const src = path.join(plugins.PLUGINS_DIR, `${name}.js`);
    if (fs.existsSync(src)) {
      fs.copyFileSync(src, path.join(dir, `${name}.js`));
...
function rollback() {
  ...
  for (const f of manifest.files) {
    const dst = path.join(plugins.PLUGINS_DIR, `${f.name}.js`);
...
// ---------- 审批队列（内存；刷新后经 /api/proposals 恢复展示） ----------
...
function addProposal(p, source) {
  ...
  const rec = { id, ...p, source: source || 'outer', old, createdAt: new Date().toISOString() };
...
// 应用一项修改：快照 → 写/删 → 热加载（失败自动回滚该文件）→ 审计
function apply(p, source) {
  ...
  try {
    ...
    const dst = path.join(plugins.PLUGINS_DIR, `${p.plugin}.js`);
    const existed = fs.existsSync(src);
    if (existed) { fs.copyFileSync(src, dst); plugins.hotReload(p.plugin); }
    else { try { fs.unlinkSync(dst); } catch {} plugins.hotUnload(p.plugin); }
...
// 手动保存/删除：用户亲手操作 = 自我批准，复用同一管线（先快照）
...
module.exports = { addProposal, listProposals, decide, rollback, manualSave, manualDelete, audit };
