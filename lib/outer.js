...
const SYSTEM_PROMPT = [
  ...
  '4. 插件文件约定：文件头注释 // @name // @desc // @essential 提供元信息；module.exports = { params: JSONSchema, run: async (args, ctx) => string }。',
...
function runOuterReal(cmd, prompt, cwd, onEvent) {
  return new Promise((resolve) => {
    ...
    const killer = setTimeout(() => { try { child.kill('SIGKILL'); } catch {} finish('执行超时（10 分钟）'); }, 10 * 60 * 1000);
