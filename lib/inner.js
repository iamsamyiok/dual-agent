...
async function chatInnerReal(cfg, messages, tools, callPlugin, onEvent) {
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    ...
    const { content, calls } = await withRetry(async () => {
      const resp = await fetch(url, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${cfg.api_key}` },
        // 上下文预算：超阈值压缩旧 tool 结果（副本发送，落盘会话保持完整）
        body: JSON.stringify({ model: cfg.model, messages: budgetMessages(messages), tools: tools.length ? tools : undefined, stream: true })
      });
...
module.exports = { chatInner, MAX_ROUNDS, sanitizeToolArguments, parseToolArgs, reassembleCalls, shouldStall, recordFail, STALL_LIMIT, budgetMessages, estimateChars };
