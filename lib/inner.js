...
async function chatInnerReal(cfg, messages, tools, callPlugin, onEvent) {
  ...
  for (let round = 0; round < MAX_ROUNDS; round++) {
    ...
    for (const c of toolCalls) {
      ...
      const t0 = Date.now();
      // 参数整体丢失（API 传输 bug）：跳过插件直接给精准重试提示，避免误导模型缩短内容绕路
      const result = (c.emptyRaw && !Object.keys(args).length)
        ? `插件 ${c.name} 调用被拒绝：本次调用的 arguments 在 API 流式传输中整体丢失（参数为空，非内容过长所致）。请原样重试同样的调用，无需缩短或修改内容。`
        : await callPlugin(c.name, args);
      const ms = Date.now() - t0;
      const ok = !/^插件.*?(加载失败|执行出错|调用被拒绝)/.test(result);
      onEvent({ type: 'tool_result', plugin: c.name, ok, result: result.slice(0, 600), ms });
...
module.exports = { chatInner, MAX_ROUNDS, sanitizeToolArguments, parseToolArgs, reassembleCalls, shouldStall, recordFail, STALL_LIMIT };
