...
async function chatInnerReal(cfg, messages, tools, callPlugin, onEvent, opts = {}) {
  ...
  for (let round = 0; round < maxRounds; round++) {
    ...
    const notes = [];
    if (usageTotals.calls > 0) notes.push(usageNoteMsg(lastUsage, usageTotals, sendChars));
    ...
    if (roundsLeft <= Math.max(2, Math.ceil(maxRounds * 0.25))) {
      notes.push({ role: 'system', content: `[轮数预算] 已用 ${round}/${maxRounds} 轮，仅剩 ${roundsLeft} 轮。立即收敛：基于已掌握的证据输出最终结论——已有的数字/事实直接用，缺失部分明确标注"未找到可靠数据"与不确定度。禁止开启新探索线或继续找更精确的数字。` });
    }
    ...
    const execOne = async (c) => {
      ...
      if (c.emptyRaw || (c.truncatedRaw && !Object.keys(args).length)) {
        ...
        if (!/^插件.*?(加载失败|执行出错|调用被拒绝)/.test(result)) transLoss.n = 0; // 任一次成功执行说明通道恢复
      }
  ...
  const tail = `已达到工具调用轮数上限（${maxRounds}），强制结束本轮。可以发"继续"让我接着完成。`;
...
module.exports = { chatInner, chatInnerReal, MAX_ROUNDS, READONLY_PLUGINS, sanitizeToolArguments, parseToolArgs, reassembleCalls, shouldStall, recordFail, STALL_LIMIT, budgetMessages, estimateChars, estimateTokens, usageNoteMsg, isMultiStepTask };
